...
public class AccountController(UserManager<UserEntity> userManager, ApplicationContext context) : Controller
{
    ...
    public async Task<IActionResult> UpdateBasicInfo(AccountDetailsViewmodel model)
    {
       ...
       { 
            ...
            if (user != null) 
            {
                ...
                if (result.Succeeded) 
                {
                    TempData["StatusMessage"] = "New basic information is updated";
                }
                else 
                { 
        ...
        if (TryValidateModel(model.Address!))
        {
            ...
            if (user != null)
            {
                try
                {
                    ...
                    TempData["StatusMessage"] = "New address information is updated";
    ...
    public async Task<IActionResult> UploadProfileImage(IFormFile file)
    {
        ...
        if (user != null && file != null && file.Length !=0)
        {
            ...
            var filePath = Path.Combine(Directory.GetCurrentDirectory(),"wwwroot/images/upload/profiles", fileName);
        ...
        {
            TempData["StatusMessage"] = "Unable to save profile image";
        }
